...
@section('content')
			...
			<ul class="nav nav-tabs mt10 mb10">
                <li ><a href="{{ route('admin.expenses.add-expenses')}}">业务支出</a></li>    
                <li class="active"><a href="{{ route('admin.stand-expenses.add-expenses')}}">独立支出</a></li>     
            </ul>
		 <form id="js-form-validate" action="{{ route('admin.stand-expenses.store')}}" method="post"  onSubmit="return false" id="search-box">
                <table class="table table-listing">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" name="amount" class="input input-small" id="total_amount" value="<?php if (isset($expensesData->amount)){echo $expensesData->amount;}?>" data-rules="required match(/^(-?\d+)(\.\d+)?$/)"  data-msg="必填 请输入数值"  maxlength="9">
                            </td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div class="center">
                    <input type="hidden" name="id" value="<?php if (isset($expensesData->id)){echo $expensesData->id;}?>" id="id">
                    <input class="btn btn-blue" type="submit" value="确定">
                    <a class="btn" href="{{route('admin.stand-expenses.list-expenses')}}">取消</a>
                </div>
            </form>
