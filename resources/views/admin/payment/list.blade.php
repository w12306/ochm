...
@section('content')
	<script>
        $.extend(ST.ACTION,{
			bExpnedList:"{{ route('admin.payment.api.list-payment-data') }}",  //获取业务支出列表接口, 
            bExpnedDelete:"{{ route('admin.payment.delete') }}" //删除业务支出接口
        });
    </script>
        ...
        <div class="pull-right">
            <a title="当前数据导出" href="javascript:;" class="btn mr5"
                    data-url="{{ route('admin.payment.export-excel') }}"
                    id="export-btn">当前数据导出</a>
        </div>
    	</div>
			<ul class="nav nav-tabs mt10 mb10">
                <li class="active"><a href="#">业务付款</a></li>    
                <li><a href="{{ route('admin.stand-payment.list') }}">独立付款</a></li>     
            </ul>
