    ...
    <form action="" method="get">
        <table class="table table-listing">
			...
			<!--<tr>
                ...
                <td>
				<label><input type="checkbox" name="contract_status[]" value="1" <?php if(!empty(Request::get('contract_status')) && in_array(1,Request::get('contract_status'))){echo 'checked="checked"'; }?> ><span class="pr10">已发合同未回</span></label>
                <label><input type="checkbox" name="contract_status[]" value="2"  <?php if(!empty(Request::get('contract_status')) && in_array(2,Request::get('contract_status'))){echo 'checked="checked"'; }?>><span class="pr10">合同已存档</span></label>
                </td>
            </tr>-->
			<tr>
                <td class="span4">确认函状态:</td>
                <td>
				<label><input type="checkbox" name="verify_type" value="1" ><span class="pr10">未发</span></label>
                                <label><input type="checkbox" name="verify_type" value="2" ><span class="pr10">已发未确认</span></label>
                                <label><input type="checkbox" name="verify_type" value="3" ><span class="pr10">已发已确认</span></label>
                </td>
            </tr>
			
			<tr>
                <td class="span4">业务状态:</td>
                <td>
				
				@foreach ($audit_status as $k=>$d)
					<?php if(!empty(Request::get('audit_status'))){ ?>
					<label><input type="checkbox" name="audit_status[]" value="{{$k}}"  <?php if(in_array($k,Request::get('audit_status'))){echo 'checked="checked"'; }?> />&nbsp;&nbsp;{{$d}}&nbsp;&nbsp;</label>
					<?php }else{ ?>
					<label><input type="checkbox" name="audit_status[]" value="{{$k}}" ><span class="pr10">{{$d}}</span></label>
					<?php } ?>
	   ...
	   @else
            @foreach ($listdata as $d)
				<tr >
					  ...
					  <font color="#FF0000">
					   <?php
					  	if(!empty($d->business->parent_contract_id)){
							echo $d->business->parent_contract_id;
						}
					  ?>
					  </font>
					  <?php $ctkey=[];?>
					  ...
					  <td>{{ $d->business->confirmstatus_text }}</td>
					  <td>
					  
					  <?php 
					  if($d->isshow==1){
					  	if($d->business->audit_status!=-1){ ?>
						    ...
						    <a href="{{ route('admin.business.create') }}/{{ $d->business->id }}" class="js-product-edit"><i hidid="" class="icon i-edit" title="编辑业务"></i></a>
						  	<?php } ?>
						  <a href="javascript:;" class="js-bus-del" data-id="{{ $d->id }}"><i hidid="" class="icon i-del" title="删除业务"></i></a>
					  
						  <?php if($d->business->audit_status==0){ ?>
						  <a href="{{ route('admin.business.business-detail', ['id' => $d->business->id]) }}" class="js-product-edit"><i hidid="" class="icon i-audit" title="审核"></i></a>
						  <?php } ?>
						  ...
						  <?php }
							...
							if($d->isshow==0){
						   ?>
						  <font color="#CCCCCC">删除时间<br>{{ $d->updated_at }}</font>
					<?php } ?>
