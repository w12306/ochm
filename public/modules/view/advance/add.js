define(function (require) {
    ...
    var tpl = require("tpl/advance/add.html");
    ...
    var View = Base.View.extend({
        ...
        checkModelLoad:function(){
            ...
            var partner_id = $(".js-partner").val(),
                ...
                // times = $(".js-times").val()||"",
                advancecash_id = $("#advancecash_id").val();
                earnestcash_id=$("#earnestcash_id").val();
            
            if(!t.model){
                t.model = new M({
                    url:ST.ACTION.advanceBusList,
                    data:{
                        partner_id:partner_id,
                        business_key:business_key,
                        earnestcash_id:earnestcash_id,
                        // times:1,
                        advancecash_id:advancecash_id
                    }
                });
            }else{
                t.model.changePars({
        ...
        errorRender:function(){
            this.$el.show().html("未查询到数据（请在下表填写发票金额拆分情况，总体金额自动加和）");
        },
        render:function(){
            var t=this,
        ...
        afterRender: function () {
            ...
            $(".js-amount-input").on("input propertychange",changeRefundTxt);

            function changeRefundTxt(){
                var dkey = $(this).data("id");
                var val = $(this).val();
                $(".js-refund-txt[data-id='"+dkey+"']").text(val);
            }
        ...
        sumAmount:function(){
            ...
            */
            ...
            $("#total_amount_refund").val(total.toFixed(2));
            $("#table-sum-amount").text(total.toFixed(2));
        },
