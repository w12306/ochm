define(function(require, exports, module) {
    ...
    (function(root) {
        ...
        _.mixin({
            ...
            // 转义参数 eg.[{name:'type',value:'1'},{name:'id',value:'1'}]  格式同$(...).serializeArray()
            ...
            truncate: function(str, num, suffix, isByte) {
                ...
                if (str) {
                    if (!isByte) {
                        ...
                        var n = Math.min(num, len),
                            arr = [];
                        len = _.byteLength(str);
                        for (var i = 0; i < len; i++) {
            ...
            interface2Event: function(name) {
                ...
                name = _.trim(name);
                if (name) {
                    var arr = name.replace(/^on/, '').match(/[A-Z][a-z]+/g);
                    if (_.indexOf(['before', 'after'], arr[0]) > -1) {
                        arr.shift();
                        result.push(_.capital(arr[0]));
                    }
            ...
            //根据列数生成矩阵
            // _.matrix([1,2,3,4,5],3) => [[1,2,3],[4,5]]
            matrix: function(arr, cols) {
                if (!_.isArray(arr)) {
                    return [];
                }
                var result = [],
            ...
            capital: function(word) {
                ...
                return word.replace(/^./, function(w) {
