			...
			'<label for="<%=controlId%>_plist_a_<%=i%>">',
          	...
          	'<%if(d.data && d.data.length>0){%>',
          		'<ul class="dropdown-submenu">',
            		...
            		'<li>',
		             	'<input type="checkbox" class="fn_<%=controlId%>_p" id="<%=controlId%>_plist_p_<%=i%>_<%=j%>" data-pid="<%=controlId%>_plist_a_<%=i%>" data-<%=config.idkey%>="<%=jd[config.idkey]%>" data-cmd="filterPVS" value=""<%if(selected.getIndex(jd[config.idkey])>-1){%> checked="checked"<%}%>/>',
		...
		'<%for(var i=0,l=data.length,d;i<l,d=data[i];i++){%>',
			'<li>',
				'<label for="<%=controlId%>_clist_p_<%=i%>">',
          		'<input type="checkbox" class="fn__p" id="<%=controlId%>_clist_p_<%=i%>" data-cmd="selectPVS" data-<%=config.idkey%>="<%=d[config.idkey]%>" data-pid="<%=controlId%>_clist_checkAll" value=""/><%=d.name%>',
        		'</label>',
		        '<%if(d.data && d.data.length>0){%>',
		        	'<ul class="dropdown-submenu">',
		        	'<%for(var j=0,jl=d.data.length,jd;i<jl,jd=d.data[j];j++){%>',
				    	'<li>',
				        	'<input type="checkbox" class="fn_<%=controlId%>_c" id="<%=controlId%>_clist_c_<%=i%>_<%=j%>" data-<%=config.idkey%>="<%=jd[config.idkey]%>" data-pid="<%=controlId%>_clist_p_<%=i%>" data-cmd="selectCity" value=""<%if(selected.getIndex(jd[config.idkey])>-1){%> checked="checked"<%}%> />',
...
ST.CitySelect = function (id, ops) {
    ...
    var _config = {
        ...
        idkey: 'id',                                    //标识键名
        template: tpl_cityselect,                  //模板(为空则为页面输出)
        template_plist: tpl_cityselect_plist,      //省份列表模板
        template_clist: tpl_cityselect_clist,      //城市列表模板
    ...
    return {
        ...
        _renderUI: function () {
			...
			*/
			...
			$('#'+id).html(_.template(tpl_cityselect,{
                controlId: id, config: c
            })).attr('class', c.className);
            return t;
        },
        ...
        fillPlist: function () {
			...
			*/
			...
			$('#'+id + '_plist').html(_.template(tpl_cityselect_plist,{
                controlId: id, config: c, data: t.datalist['plist'], selected: filteredPVS
            }))
            //判断是否载入clist
        ...
        _fillClist:function(pids){
			...
			*/
			$('#'+id + '_clist').html(_.template(tpl_cityselect_clist,{
                controlId: id, config: c, data: _data, selected: _selected
            }))
