...
class PartnersController extends Controller
{
    ...
    private function getPartnerListData($request,$limit=""){
        ...
        if(empty($limit)){
            return  $this->partnersRepository->with(['company'])
                    ->applyWhere([
                        function ($query){
    ...
    public function getList(Request $request)
    {
        ...
        $company_type=PartnersModel::$typeArray;

        return view('admin.partners.list',['listdata'=>$listdata,
    ...
    public function storePartner(Request $request){
        ...
        $this->checkPartnerName($data['company_name'],$id);//检查合作方名称
        if(!empty($id)){//编辑
            $this->updatePartner($id,$data);
            return $this->ajaxSuccess('更新合作方成功！', [
                    'url' =>route('admin.toolbox.partner-list')
            ]);
            exit;
        }
        $partner= $this->partnersRepository->create($data);

        $this->actionLog->log('partner.create',
                Admin::user()->id,
                [  'id'             =>$partner->id,
    ...
    public function updatePartner($id,$data){
        ...
        $this->actionLog->log('partner.edit',
                Admin::user()->id,
                [  'id'             =>$id,
                   'company_name'   =>$data['company_name'],
                ]
